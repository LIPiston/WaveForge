...
export default function ModeSelectionPanel({
  ...
  const background = currentMode === 'minimal'
  ...
  return createPortal(
    ...
    >
      <div
        className="relative flex items-start justify-center overflow-hidden border-b border-white/15 px-8 pt-4 shadow-[0_20px_70px_rgba(0,0,0,0.34)]"
        style={{ height: MODE_SELECTION_PANEL_HEIGHT, background, backgroundColor: '#080b12' }}
      ...
      <AnimatePresence>
        {showCustomize && (
          <motion.div
            ...
            data-mode-customize
            className="absolute right-8 top-[204px] z-40 w-64 rounded-2xl border border-white/15 bg-[#0c0e1a]/[0.97] p-2 shadow-[0_18px_50px_rgba(0,0,0,0.55)] backdrop-blur-2xl"
            style={{ willChange: 'transform, opacity' }}
          >
            <p className="px-2 pb-1.5 pt-1 text-[11px] font-semibold tracking-[0.08em] text-white/55">显示 / 隐藏模式</p>
            {ALL_MODES.map((mode) => {
              const isVisible = effectiveVisibleModes.includes(mode)
              ...
              return (
                ...
                >
                  <span className="flex flex-col leading-tight">
                    <span className="text-sm font-medium text-white/95">{MODE_NAMES[mode]}</span>
      ...
      <div className="flex justify-center -mt-px">
        <button
          ...
          aria-label="收起模式选择"
          onClick={onClose}
          className="group relative flex h-9 w-[200px] items-center justify-center overflow-hidden rounded-b-2xl border border-t-0 border-white/25 bg-white/[0.09] shadow-[0_12px_34px_rgba(0,0,0,0.24),inset_0_-1px_0_rgba(255,255,255,0.08)] transition-colors hover:bg-white/[0.16]"
        >
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.035))] backdrop-blur-xl backdrop-saturate-150"
          />
          <span
            aria-hidden="true"
          ...
          />
          <svg className="relative z-10 h-6 w-6 text-white/95 drop-shadow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
