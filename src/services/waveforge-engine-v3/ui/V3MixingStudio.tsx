/**
 * WaveForge v3 调音室 UI —— 主面板（V3MixingStudio）
 *
 ...
 *
 * 融合接线（WaveForge 侧）：
 *   const bridge = createV3UiBridge(engine, ctx.sampleRate)
 *   <V3MixingStudio bridge={bridge} playerTheme={theme} onClose={...} ... />
 * 详见 docs/UI_GUIDE.md。
 */
...
export type V3EngineVersion = 'v1' | 'v2' | 'v3'
...
export interface V3MixingStudioProps {
  ...
  /** 当前引擎版本（切换入口显示用） */
  engineVersion?: V3EngineVersion
  /** 请求切换引擎（App 负责热/冷切换与弹窗） */
  onSwitchEngine?: (version: V3EngineVersion) => void
...
export default function V3MixingStudio({ bridge, onClose, playerTheme, anchorRect, engineVersion = 'v3', onSwitchEngine, exportWav = null, exporting = false }: V3MixingStudioProps) {
  const theme = useV3Theme(playerTheme)
