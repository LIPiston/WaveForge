...
// ---------------------------------------------------------------------------
// 真实 KEMAR 网格（球谐插值用例；hrtf-data/grid.bin 缺失 → 该组跳过）
...
try {
  ...
  gridReason = `hrtf-data/grid.bin 缺失或损坏（${String(err)}）`
}
if (!realGrid) {
  console.warn(`[tsBackend.test] 跳过球谐插值组：${gridReason}`)
}

// ---------------------------------------------------------------------------
// wasm 产物加载（getHrir 跨后端对拍组用；缺失 → 该组跳过）
// ---------------------------------------------------------------------------
...
if (!wasmBytes) {
  console.warn(`[tsBackend.test] 跳过 getHrir 跨后端对拍组：${wasmReason}`)
}
...
// ---------------------------------------------------------------------------
describe.skipIf(!realGrid)('TsConvolverBackend：球谐插值（hrtfInterp: spherical）', () => {
  const grid = realGrid as HrtfGrid
  const SPK: Array<[number, number]> = [
    [0, 0],
    [30, 0],
  ] // 网格点方向（最近邻与球谐可对比）

  const spkConfig = (interp: 'nearest' | 'spherical'): SpatialRenderConfig => ({
    speakers: SPK.map(
      ([azDeg, elDeg], i): VirtualSpeaker => ({
        channel: i % 2, // 0=L、1=R（与 wasmBackend 对拍组同布局）
        azimuthDeg: azDeg,
        elevationDeg: elDeg,
        distance: 1.5,
        gain: 1,
  ...
  function render(interp: 'nearest' | 'spherical'): { outL: Float32Array; outR: Float32Array } {
    const b = new TsConvolverBackend()
    b.loadHrtf(grid)
    b.setConfig(spkConfig(interp))
    const N = 4096
    const inL = new Float32Array(N)
    const inR = new Float32Array(N)
    for (let i = 0; i < N; i++) {
      inL[i] = 0.4 * Math.sin((2 * Math.PI * 10 * i) / FS)
      inR[i] = 0.4 * Math.sin((2 * Math.PI * 7 * i) / FS)
    }
    const outL = new Float32Array(N)
    const outR = new Float32Array(N)
    b.processStereo(inL, inR, outL, outR)
  ...
  it('spherical 模式：0°/30° 扬声器输出非零（球谐 HRIR 装载生效）', () => {
    const { outL, outR } = render('spherical')
    let m = 0
    for (let i = PART; i < 4096; i++) {
      m = Math.max(m, Math.abs(outL[i]), Math.abs(outR[i]))
    }
    expect(m).toBeGreaterThan(0.01) // 湿路非零（对比 10Hz 正弦幅度 0.4）
  })

  it('spherical 与 nearest 在网格点方向上接近（容差按拟合残差放宽）', () => {
    // 拟合残差实测：方向 (0,0) 逐样本最大差 36.5% 峰值、(30,0) 17.9% 峰值
    // （hrtfInterp.test.ts ① 注释）。卷积输出为 HRIR 差与输入的卷积，
    // 实测两后端输出最大绝对差 ≈ 0.006（输出 RMS ≈ 0.09，即 ≈7%）。
    // 断言按 3× 实测余量放宽：L2 相对差 < 0.2。
    const nn = render('nearest')
    const sp = render('spherical')
    const START = PART + 128 // 跳过延迟空白与滤波暂态
    let d2 = 0
    let n2 = 0
    let maxD = 0
    let maxN = 0
    for (let i = START; i < 4096; i++) {
      const dl = sp.outL[i] - nn.outL[i]
      const dr = sp.outR[i] - nn.outR[i]
      d2 += dl * dl + dr * dr
      n2 += nn.outL[i] * nn.outL[i] + nn.outR[i] * nn.outR[i]
      maxD = Math.max(maxD, Math.abs(dl), Math.abs(dr))
      maxN = Math.max(maxN, Math.abs(nn.outL[i]), Math.abs(nn.outR[i]))
    }
    expect(Math.sqrt(d2 / n2)).toBeLessThan(0.2) // L2 相对差（实测 ≈ 0.04）
    expect(maxD).toBeLessThan(0.2 * maxN) // 峰值相对差（实测 ≈ 0.07）
  })

  it('块连续性：spherical 模式 128 分块与整块 4096 输出一致（逐样本 ≤ 1e-6）', () => {
    const run = (chunked: boolean): { outL: Float32Array; outR: Float32Array } => {
      const b = new TsConvolverBackend()
      b.loadHrtf(grid)
      b.setConfig(spkConfig('spherical'))
      const N = 4096
      const inL = new Float32Array(N)
      const inR = new Float32Array(N)
      for (let i = 0; i < N; i++) {
...
//     spherical ≤ 1e-5——Rust 侧 spatial_get_hrir 与 build_speaker 装载分支同源）。
// ---------------------------------------------------------------------------
describe.skipIf(!realGrid)('TsConvolverBackend：getHrir（§3.2 契约）', () => {
  const grid = realGrid as HrtfGrid
  // 方向集：网格点 + 离网格（最近邻/球谐映射）+ 环绕（-180/180 相邻）+ 高仰角
  ...
  it('nearest：返回与网格该方向 HRIR 逐位一致（与 setConfig 装载分支同路径）', () => {
    const b = new TsConvolverBackend()
    b.loadHrtf(grid)
  ...
  it('spherical：与 hrtfInterp.sphericalHrtf 输出逐位一致（同一函数同一路径）', () => {
    const b = new TsConvolverBackend()
    b.loadHrtf(grid)
    b.setConfig(makeConfig([], { hrtfInterp: 'spherical' }))
    ...
    for (const [az, el] of DIRS) {
      ...
      for (let i = 0; i < grid.hrirLength; i++) {
        expect(left[i]).toBe(refL[i])
        expect(right[i]).toBe(refR[i])
      }
    }
  })

  it('未 loadHrtf 时抛错（防御分支）', () => {
    const b = new TsConvolverBackend()
    expect(() => b.getHrir(0, 0)).toThrow()
  })

  describe.skipIf(!wasmBytes)('与 WasmHrtfBackend.getHrir 对拍（同一网格/模式）', () => {
    const bytes = wasmBytes as Uint8Array

    /** 构造 wasm 后端取 HRIR 的闭包（nearest/spherical 经 setConfig 设插值模式；
     *  最小 1 只扬声器配置——空配置 spatial_alloc(0) 返回 null 会抛错） */
    function wasmGetHrir(
      interp: 'nearest' | 'spherical',
    ): (az: number, el: number) => { left: Float32Array; right: Float32Array } {
      const b = new WasmHrtfBackend(bytes)
      b.loadHrtf(grid)
      b.setConfig(
        makeConfig([{ channel: 0, azimuthDeg: 0, elevationDeg: 0, distance: 1.5, gain: 1, size: 0 }], {
          hrtfInterp: interp,
        }),
      )
      return (az, el) => b.getHrir(az, el)
    }

    it('nearest：与 TS getHrir 逐位一致（同一网格查表路径，Rust 与 TS 最近邻同索引）', () => {
      const w = wasmGetHrir('nearest')
      const t = new TsConvolverBackend()
      t.loadHrtf(grid)
      for (const [az, el] of DIRS) {
        const a = w(az, el)
        const b = t.getHrir(az, el)
        for (let i = 0; i < grid.hrirLength; i++) {
    ...
    it('spherical：与 TS getHrir 对拍 ≤ 1e-5（Rust SH 拟合 vs hrtfInterp，含离网格方向）', () => {
      const w = wasmGetHrir('spherical')
      const t = new TsConvolverBackend()
      t.loadHrtf(grid)
      t.setConfig(makeConfig([], { hrtfInterp: 'spherical' }))
      for (const [az, el] of DIRS) {
        const a = w(az, el)
        const b = t.getHrir(az, el)
        let maxD = 0
...
// ---------------------------------------------------------------------------
// f32/f64 精度边界 TS 自洽回归（O1 审计 7.3 — 对拍测试缺口，TS 侧镜像）：
// 与 wasmBackend.test.ts 同名 describe 镜像——同配置（64 扬声器 / 大距离 /
// 窄带相干输入）下 TsConvolverBackend 自洽：分块 vs 整块逐位一致、输出有限非零。
// 验证 1.1（空气吸收状态 f32 截断）+ 1.4（distance Math.fround）修复后
// TS 侧在这些边界场景仍稳定（无 NaN/发散/块间不一致）。
// ---------------------------------------------------------------------------
describe('TsConvolverBackend：f32/f64 精度边界 TS 自洽（64 扬声器 / 大距离 / 窄带相干）', () => {
  const TOTAL = 4233
  const CHUNKS = [128, 511, 1024, 1024, 1024, 522] // 与 wasmBackend 对拍组同分块（不收缩）

  /** 64 扬声器均匀环布（az -180..175 步 5.6°、el 0、dist 1.5、gain 1）——与 wasmBackend 镜像 */
  function speakers64(): VirtualSpeaker[] {
    const arr: VirtualSpeaker[] = []
    for (let i = 0; i < 64; i++) {
      arr.push({
        channel: i % 2,
  ...
  /** 固定种子随机输入（与 wasmBackend 同种子，便于跨文件交叉对照） */
  function randomInputs(seed: number, n: number): { inL: Float32Array; inR: Float32Array } {
    const inL = new Float32Array(n)
    const inR = new Float32Array(n)
  ...
  /** 单频正弦输入（窄带相干；幅度 0.5） */
  function sineInputs(freqHz: number, n: number): { inL: Float32Array; inR: Float32Array } {
    const inL = new Float32Array(n)
    const inR = new Float32Array(n)
    for (let i = 0; i < n; i++) {
      inL[i] = 0.5 * Math.sin((2 * Math.PI * freqHz * i) / FS)
      inR[i] = 0.5 * Math.sin((2 * Math.PI * freqHz * i) / FS + Math.PI / 3)
    }
    return { inL, inR }
  }

  /** 固定种子 PRNG（mulberry32，与 wasmBackend.test.ts 同实现——跨文件同序列） */
  ...
  /** 分块渲染 vs 整块渲染逐位一致性（块连续性回归，逐样本 ≤ 1e-6） */
  function chunkedVsWhole(
    config: SpatialRenderConfig,
    inputs: { inL: Float32Array; inR: Float32Array },
  ): number {
    const { inL, inR } = inputs
    // 分块渲染
    const bChunk = new TsConvolverBackend()
    bChunk.loadHrtf(deltaGrid())
    bChunk.setConfig(config)
    const outChunk: Float32Array[] = [new Float32Array(TOTAL), new Float32Array(TOTAL)]
    let off = 0
    for (const c of CHUNKS) {
      const n = Math.min(c, TOTAL - off)
      bChunk.processStereo(
        inL.subarray(off, off + n),
        inR.subarray(off, off + n),
        outChunk[0].subarray(off, off + n),
        outChunk[1].subarray(off, off + n),
      )
      off += n
    }
    // 整块渲染（一块喂入 TOTAL，内部按 MAX_FRAME=4096 自动分块）
    const bWhole = new TsConvolverBackend()
    bWhole.loadHrtf(deltaGrid())
    bWhole.setConfig(config)
    const outWhole: Float32Array[] = [new Float32Array(TOTAL), new Float32Array(TOTAL)]
    bWhole.processStereo(inL, inR, outWhole[0], outWhole[1])
    let m = 0
    for (let i = 0; i < TOTAL; i++) {
      m = Math.max(m, Math.abs(outChunk[0][i] - outWhole[0][i]), Math.abs(outChunk[1][i] - outWhole[1][i]))
    }
    return m
  }

  it('64 扬声器：分块 vs 整块逐位一致（≤ 1e-6）、输出有限非零', () => {
    // 高扬声器数下 f32 累加顺序敏感度上升——验证 1.1（airState 每样本截断）
    // + 1.4（distance fround）修复后 TS 侧仍块间确定性（状态跨块一致）。
    const config = makeConfig(speakers64())
    const inputs = randomInputs(0x64a1, TOTAL)
    expect(chunkedVsWhole(config, inputs)).toBeLessThanOrEqual(1e-6)
    // 输出有限非零（64 路卷积累加实际生效，非空转）
    const b = new TsConvolverBackend()
    b.loadHrtf(deltaGrid())
    b.setConfig(config)
    const outL = new Float32Array(TOTAL)
    const outR = new Float32Array(TOTAL)
    b.processStereo(inputs.inL, inputs.inR, outL, outR)
    let m = 0
  ...
  it('大距离（dist=50m，α→1 状态累积最敏感）：分块 vs 整块逐位一致、输出有限非零', () => {
    // dist=50m → fc≈78Hz、α≈0.0102（小 α 下状态 y 跨块累积——验证 1.1 修复
    // 在最敏感场景仍稳：每样本截断 vs 原块内 f64 carry 在大距离下偏差最大）。
    const config = makeConfig([
    ...
    const inputs = randomInputs(0x50d1, TOTAL)
    expect(chunkedVsWhole(config, inputs)).toBeLessThanOrEqual(1e-6)
    // 输出有限非零（远距离增益 1/50=0.02，仍 > 1e-4）
    const b = new TsConvolverBackend()
    b.loadHrtf(deltaGrid())
    b.setConfig(config)
    const outL = new Float32Array(TOTAL)
  ...
  it('窄带相干输入（440Hz 正弦，复乘 f32/f64 最坏情况）：分块 vs 整块逐位一致、输出有限非零', () => {
    // 单频正弦 → 卷积谱集中于单一频率附近，复乘 r1·r2−i1·i2 在窄带下 f32/f64
    // 差异最坏（无宽带平均抵消）——验证 TS 侧仍块间确定性 + 有限非零。
    const config = makeConfig([
      { channel: 0, azimuthDeg: 0, elevationDeg: 0, distance: 1.5, gain: 1, size: 0 },
      { channel: 1, azimuthDeg: 30, elevationDeg: 0, distance: 1.5, gain: 1, size: 0 },
    ])
    const inputs = sineInputs(440, TOTAL)
    expect(chunkedVsWhole(config, inputs)).toBeLessThanOrEqual(1e-6)
    // 输出有限非零（440Hz 正弦经 HRTF 卷积后双耳仍有显著响应）
    const b = new TsConvolverBackend()
    b.loadHrtf(deltaGrid())
    b.setConfig(config)
    const outL = new Float32Array(TOTAL)
